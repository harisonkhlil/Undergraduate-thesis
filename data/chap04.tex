...
从图中可以看到，每一层在时刻$t$的值，仅与其上一层在时刻$t, t-1, \dots$的值相关，这符合因果卷积的原理。此外，信息的提取过程在每一层中都是以跳跃式进行的，且每层的空洞率(dilated rate)都以2的指数形式增加，展现了空洞卷积的特点。同时，由于采纳了空洞卷积，每个隐藏层都需要进行填充(padding)，其大小由公式$padding = (kernel_size - 1)$确定。
